...
// https://github.com/replayfy/ios-sdk  (from 0.0.3)

dependencies: [
    .package(url: "https://github.com/replayfy/ios-sdk.git", from: "0.0.3")
],
targets: [
    .target(
        name: "YourApp",
...
<Warning>
  A few methods are on the public surface but are no-ops in this release, pending an engine follow-up: `cancelSession()`, `optOutOfSchematicRecordings(_:)` / `isOptedOutOfSchematicRecordings`, `urlForCurrentSession()` / `urlForCurrentUser()`, `enableAdvancedGestureRecognizer(_:)`, `allowShortBreakForAnotherApp(_:windowSeconds:)`, and `setMultiSessionRecord(_:)`. Calling them is safe but has no effect.
</Warning>
