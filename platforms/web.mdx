...
Include `picture` (or its alias `avatar`) with a public `https` image URL — either top-level or inside `customProps` — to set the user's avatar in the recording header and Users list; non-URL values are ignored.
